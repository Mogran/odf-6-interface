...
//	printf("numbers of dfu packets is %d.................. \n", calced_packet_num);

	for(ofs = 0; ofs < calced_packet_num - 1; ofs++){
		memset(&data_packet[ofs][0], 0xff, 512);
		memcpy(&data_packet[ofs][0], &dfu.packets[ofs*512], 512);
//		debug_net_printf(&data_packet[ofs][0], 512, 60001);
	}

	memset(&data_packet[ofs][0], 0xff, 512);
	memcpy(&data_packet[ofs][0], &dfu.packets[ofs*512], dfu_file_size - (ofs*512));

	if(!(calced_packet_num % 2)){
		dfu.data_packet_num = calced_packet_num;
	}else{
		memset(&data_packet[ofs + 1][0], 0xff, 512);
		dfu.data_packet_num = calced_packet_num + 1;
	}
...
void *odf_cmd_0x1107_download_software_upgrade_file_pthread(void *arg)
{
	...
	for(;;){
		...
		nbytes = recv(dfu.sockfd, &dfu.packet_i3_data[0], 2048, 0);
		if(nbytes < 0){
//			printf("%s:%d recv errno = %d .......\n", __func__, __LINE__, errno);
//			if(errno == -EAGAIN)
//				printf("%s:%d recv errno = -EGAIN .......\n", __func__, __LINE__);
//			else
//				printf("%s:%d recv abort!!!  .......\n", __func__, __LINE__);
		...
		if(retval == 0){
		//	printf("%s:%d recv crc16 abort!!! bytes = %d .......\n", __func__, __LINE__, nbytes);
			retval = ftp_client(&dfu.packet_lo_data[0], 0, dfu.sockfd, 0);	
			if(retval == DFU_REQUEST_CMD_TYPE_0x03){
//				printf("%s:%d exit pthread ... [done]\n", __func__, __LINE__);
				break;
			}

		}else{
		//	printf("%s:%d recv crc16 normal!!! bytes = %d .......\n", __func__, __LINE__, nbytes);

			retval = ftp_client(&dfu.packet_lo_data[0], 0, dfu.sockfd, 1);	
			if(retval == DFU_REQUEST_CMD_TYPE_0x03){
				break;
			}
		}
...
static uint8_t dfu_request_cmd_type_0x01_handle_routine(uint8_t *htmldat, int fversion, int sockfd, int crc16_stat)
{
	int ret = 0;
	uint16_t ofs = 0, crc16_val = 0;
	pthread_t pthid;
...
//		printf("%s:%d send data success .......\n", __func__, __LINE__);
	...
	ret = pthread_create(&pthid, NULL, odf_cmd_0x1107_download_software_upgrade_file_pthread, 0);
	if(ret == -1){
//		printf("%s:%d pthread create fail ...n", __func__, __LINE__);
		return ret;
	}	
...
static uint8_t dfu_request_cmd_type_0x02_handle_routine(uint8_t *htmldat, int fversion, int sockfd, int crc16_stat)
{
	uint16_t ofs = 0, crc16_val = 0;
	uint32_t idex = 0, nbytes = 0;
	int ret = 0;
...
//		printf("%s:%d current packet num = %d ...\n", __func__, __LINE__, dfu.recv_packet_num);
//		debug_net_printf(htmldat, 1100, 60001);
	}
...
static uint8_t dfu_request_cmd_type_0x03_handle_routine(uint8_t *htmldat, int fversion, int sockfd, int crc16_stat)
{
	uint16_t ofs = 0, crc16_val = 0;
	uint16_t ret = 0;
...
//			debug_net_printf(&dfu.packets[0], 225*1021, 60001);
			ret = 0x1;
		}else{
			ret = 0x0;
//			debug_net_printf(&dfu.packets[0], 225*1021, 60001);
...
static uint8_t dfu_request_cmd_type_0x04_handle_routine(uint8_t *htmldat, int fversion, int sockfd,  int crc16_stat)
{
	uint16_t ofs = 0, crc16_val = 0;
	uint16_t ret = 0;
	...
	if(dfu.type == 0x01){
		if(cf8051.update == UPDATE_NOW)
			ret = UPDATE_NOW;	
		else if(cf8051.update == UPDATE_DONE)
			ret = cf8051.update_result;	
	}

	if(dfu.type == 0x02){
		if(sim3u146.update == UPDATE_NOW)
			ret = UPDATE_NOW;	
		else if(sim3u146.update == UPDATE_DONE)
			ret = sim3u146.update_result;	
	}

	if(dfu.type == 0x03){
		if(dfu.update_result)
			ret = 0x01;//update failed.
		else
			ret = 0x00;
	}
 ...
 *
 */
int ftp_client(unsigned char *htmldat, int fversion, int fd, int crc16_verify_result)	
{
	int ret = 0;

	switch(htmldat[20]){
	case DFU_REQUEST_CMD_TYPE_0x01:	
	//	printf("recv 0x01 cmd ....  \n");
		dfu_request_cmd_type_0x01_handle_routine(htmldat, fversion, fd, crc16_verify_result);
		ret = DFU_REQUEST_CMD_TYPE_0x01;
	break;
	case DFU_REQUEST_CMD_TYPE_0x02:
	//	printf("recv 0x02 cmd ....  \n");
		dfu_request_cmd_type_0x02_handle_routine(htmldat, fversion, fd, crc16_verify_result);	
		ret = DFU_REQUEST_CMD_TYPE_0x02;
	break;	
	case DFU_REQUEST_CMD_TYPE_0x03:
	//	printf("recv 0x03 cmd ....  \n");
		dfu_request_cmd_type_0x03_handle_routine(htmldat, fversion, fd, crc16_verify_result);	
		ret = DFU_REQUEST_CMD_TYPE_0x03;
	break;
	case DFU_REQUEST_CMD_TYPE_0x04:
	//	printf("recv 0x04 cmd ....  \n");
		dfu_request_cmd_type_0x04_handle_routine(htmldat, fversion, fd, crc16_verify_result);	
		ret = DFU_REQUEST_CMD_TYPE_0x04;
	break;
	default:
