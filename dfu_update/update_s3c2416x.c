...
int update_s3c2416x(unsigned char *webmaster_data, int socket_fd)
{
	...
	if(fversion%2){
		...
		fp = fopen("/var/yaffs/odf_china_Unicom_2", "wb+");
		if(fp == NULL){
			return -1;
		}else{
			for(packet_idex = 0; packet_idex < dfu.data_packet_num; packet_idex++){
				fwrite( &dfu.packets[packet_idex*1021], 1021, 1, fp);
				usleep(2000);
			}
			fclose(fp);
			fp = NULL;
			system(" chmod 775 /var/yaffs/odf_china_Unicom_2");
		}
		...
		fp = fopen("/var/yaffs/odf_china_Unicom_1", "wb+");
		if(fp == NULL){
			return -1;
		}else{
			for(packet_idex = 0; packet_idex < dfu.data_packet_num; packet_idex++){
			...
			fclose(fp);
			fp = NULL;
			system(" chmod 775 /var/yaffs/odf_china_Unicom_1");
		}
