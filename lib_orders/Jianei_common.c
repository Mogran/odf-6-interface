...
void orderOperations_wrong_mark_input(uint8_t *src, uint8_t type, int sockfd)
{
	uint16_t crc16_val = 0x0, send_length = 0;
	uint8_t local_tmp_buf[40];
	uint8_t i3_tmp_buf[40];
	uint8_t ofs = 0;

	local_tmp_buf[ofs++] = 0x7e;
	local_tmp_buf[ofs++] = 0x00;
	local_tmp_buf[ofs++] = 0x10;
	
	memset(&local_tmp_buf[ofs], 0x0, 14);
	ofs += 14;	
	
	local_tmp_buf[ofs++] = 0x80;
	local_tmp_buf[ofs++] = 0x11;
	local_tmp_buf[ofs++] = 0x00;
	local_tmp_buf[ofs++] = 0xee;

	memcpy(&local_tmp_buf[ofs], &src[5], 8);

	crc16_val = crc16_calc(local_tmp_buf, ofs - 1);
	local_tmp_buf[ofs++] = crc16_val & 0xff;
	local_tmp_buf[ofs++] = (crc16_val >> 8) & 0xff;

	send_length = local_data_transform_to_i3_format_data(&local_tmp_buf[0], &i3_tmp_buf[0], ofs);
	
	if(Jianei_Order.ord_type == ORDER_NET){
		if(send(Jianei_Order.ord_fd, &i3_tmp_buf, send_length, 0) == -1){
			
		}

	}else{
			/* 发送给手机*/
		write(uart1fd, &i3_tmp_buf, send_length); 
	}
}
...
void orderoperations_failure_and_reply_to_mobile(uint8_t *Buffer, uint8_t orderCmd, uint8_t errCode)
{
#if 0	
