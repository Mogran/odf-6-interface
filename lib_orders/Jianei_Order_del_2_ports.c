...
void Jianei_Order_del_2_ports(unsigned char *src, unsigned char data_source, int sockfd)
{
	...
	if(data_source == DATA_SOURCE_MOBILE || data_source == DATA_SOURCE_NET)
	{
		if(0xa2 != src[21] && 0x9a != src[21]){
			...
			if(!myStringcmp(&boardcfg.box_id, &src[WEBMASTER_DATA_UUID_INDEX], WEBMASTER_DATA_UUID_LENGTH)){
				orderoperations_failure_and_reply_to_mobile(CMD_DEL_2_PORTS, 0x01);
				return ;
			}
			...
			if(Jianei_Order.eid_port1[0] == Jianei_Order.eid_port2[0]){
				...
				Jianei_Order.zkb_rep0d[4] = CMD_DEL_2_PORTS;
			...
			if(!module_order_tasks_to_sim3u1xx_board()){
				orderoperations_failure_and_reply_to_mobile(CMD_DEL_2_PORTS, 0x03);
				return ;
			}
	...
	}else if(data_source == DATA_SOURCE_UART)
	{
		if(0x01 == src[5])
		{
            ...
            if(myStringcmp(&Jianei_Order.eid_port1[0], &src[7], 3)){
		   		Jianei_Order.eid_port1[40] = 0x01;
		    }else if(myStringcmp(&Jianei_Order.eid_port2[0], &src[7], 3)){
		   		Jianei_Order.eid_port2[40] = 0x01;
		    }

			/* 如果两端都拔出了标签，而且是一致的，那就给这两端口发自动确认命令*/
			if(Jianei_Order.eid_port1[40] == Jianei_Order.eid_port2[40])
			{
				if(Jianei_Order.eid_port1[0] == Jianei_Order.eid_port2[0]){
					/* 同框的数据*/
					Jianei_Order.zkb_rep0d[0] = 0x7e;
		...
		{
			/* 拔出的端口重新插入了，判断是eid_port1 还是eid_port2 */
            if(myStringcmp(&Jianei_Order.eid_port1[0], &src[7], 3)){
		   		Jianei_Order.eid_port1[40] = 0x02;
		    }else if(myStringcmp(&Jianei_Order.eid_port2[0], &src[7], 3)){
		   		Jianei_Order.eid_port2[40] = 0x02;
		    }
		...
		}else if(0x0a == src[5])
		{
			...
			if(0x0a == Jianei_Order.eid_port1[40] && 0x0a == Jianei_Order.eid_port2[40]){
				/* 工单完成 */
				orderoperations_success_and_reply_to_mobile(NULL, CMD_DEL_2_PORTS, 0x00);
				...
				Jianei_Order.eid_port2[3] = DYB_PORT_NOW_USING;
				...
				orderoperations_failure_and_reply_to_mobile(CMD_DEL_2_PORTS, 0x07);
			}
