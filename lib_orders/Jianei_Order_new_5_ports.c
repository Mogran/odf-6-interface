...
void Jianei_Batch_new_5_ports(unsigned char *src, unsigned char data_source, int sockfd)
{
	...
	}else if(data_source == DATA_SOURCE_UART)
	{
		...
		{/* 新建中的端口有拔出动作*/
			...
			if(Jianei_Batch.ddk_port_stat[patch_idex] == DYB_PORT_STAT_MARK_OFFLINE && Jianei_Batch.ydk_port_stat[patch_idex] == DYB_PORT_STAT_MARK_OFFLINE)
			{
...
static uint8_t Preparing_record_5_ports_relationship(unsigned char *src)
{
	...
	Z_Start_idex = (datalen - A_Start_idex)/2 + WEBMASTER_DATA_PORTA_INDEX;
	...
	for(datalen = 0; datalen < 5; datalen++){
		...
		debug_net_printf(&Jianei_Batch.ydk_port_info[datalen][0], 4, 60001);
		debug_net_printf(&Jianei_Batch.ddk_port_info[datalen][0], 4, 60001);
		debug_net_printf(&Jianei_Batch.ydk_port_stat[datalen], 5, 60001);
		debug_net_printf(&Jianei_Batch.ddk_port_stat[datalen], 5, 60001);
		debug_net_printf(&Jianei_Batch.ydk_port_eids[datalen][0], 32, 60001);
		debug_net_printf(&Jianei_Batch.ddk_port_eids[datalen][0], 32, 60001);
	}
#endif
